...
func _physics_process(_delta):
	if not object_grabbed and $TextTimer.is_stopped() and get_collider() is RigidBody and get_collider().mass <= mass_limit || $TextTimer.is_stopped() and get_collider() is Area and get_collider().has_method('interact'):
		grab_text_appears()
	else:
	...
	if Input.is_key_pressed(KEY_E) or Input.is_joy_button_pressed(0, JOY_XBOX_Y):
		if can_use:
			...
			if not object_grabbed:
				...
				elif get_collider() is Area and get_collider().has_method('interact'):
